describe Parser do
  describe '::tokenize' do
    ...
    it 'returns an array' do
      expect(Parser::tokenize(simple_string)).to be_an(Array)
    end

    it 'splits the input on spaces' do
      expect(Parser::tokenize(simple_string).count).to eq(2)
    end

    it 'strips trailing whitespace' do
      result = Parser::tokenize(simple_string + ' ')

      expect(result[1]).to eq('string')
    end

    it 'separates tokens' do
      expect(Parser::tokenize(string_with_tokens).count).to eq(7)
    end
  end
  ...
  describe '::categorize' do
    context 'when the input is a string' do
      ...
      it 'returns a literal token' do
        expect(Parser::categorize(input).type).to eq(:literal)
      end

      it 'returns a token with the string value' do
        expect(Parser::categorize(input).value).to eq('string')
      end
    end
    ...
    context 'when the input is an integer' do
      ...
      it 'returns a token with the integer value' do
        expect(Parser::categorize(input).value).to eq(1)
      end
    end
    ...
    context 'when the input is an identifier' do
      ...
      it 'returns an identifier token' do
        expect(Parser::categorize(input).type).to eq(:identifier)
      end

      it 'returns a token with the identifier value' do
        expect(Parser::categorize(input).value).to eq('identifier')
      end
    end
  end
    ...
    context 'when the input contains tokens' do
      context 'when the input is a single atom' do
        ...
        it 'returns a Token' do
          expect(Parser::parse(input)).to be_a(Token)
        end

        it 'returns the atom as a categorized token' do
          expect(Parser::parse(input).type).to eq(:literal)
        end
      end
      ...
      context 'when the input contains multiple atoms' do
        ...
        it 'returns an array containing the atoms' do
          expect(Parser::parse(input).count).to eq(2)
        end

        it 'categorizes the atoms' do
          expect(Parser::parse(input).all? { |t| t.type == :literal }).to eq(true)
        end
      end
